```{r reg, cache=cacheon, fig.cap="Registration over time", fig.width=9.5, fig.height=7}
...
axis(2,
  at = seq(0, 20000, 2000),
  fnbm(seq(0, 20000, 2000)),
  cex.axis = cexmy, las = 2
)

axis(2, at = 10000, "Number of registrations", cex.axis = cexmy, tick = FALSE, line = 3)

axis(1, at = 1:length((global_year - 5):global_year), labels = (global_year - 5):global_year, cex.axis = cexmy)

legend(x = 0.9, y = 20500, all %>% pull(byvarname),
  col = global_cols[c(1, 2, 3, 4, 5, 6, 7, 8)],
  lwd = 3, pch = 19, bty = "n", cex = cexmy
)
```
...
default_kable(all, caption = "Registration over time", escape = F, align = c("l", rep("c", ncol(all) - 1)))
```
